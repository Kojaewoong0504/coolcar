...
const available = lookupDoorGuide({ line: '1호선', toStation: '서울역', direction: '시청', goal: 'FINAL_EXIT' });
...
const noDirection = lookupDoorGuide({ line: '1호선', toStation: '서울역', goal: 'FINAL_EXIT' });
assert(noDirection.status === 'needs_direction', 'directional data without direction should need direction');

const unsupported = lookupDoorGuide({ line: '2호선', toStation: '강남역', direction: '내선', goal: 'FINAL_EXIT' });
...
const guidance = buildRouteGuidance(request, car);
...
const ambiguousGuidance = buildRouteGuidance(ambiguousRequest, car);
